...
public static class TrainingArenaBuilder
{
    ...
    private static NpcController CreateEnemyPrefab()
    {
        ...
        Set(npc, "config.EnemyMaxHp", 60); Set(npc, "config.AggroRange", 30f); Set(npc, "config.ShowEnemyHealthBar", true);
        ...
        var attacker = go.AddComponent<CombatAttacker>(); Set(attacker, "config.UsePlayerInput", false); Set(attacker, "config.AttackDuration", 0.35f);
        Set(attacker, "config.AttackCooldown", 0.6f); Set(attacker, "config.AttackDamage", 5);
        ...
        Set(ai, "config.ArenaX", ArenaBounds.x); Set(ai, "config.ArenaY", ArenaBounds.y);
        Set(ai, "config.ArenaWidth", ArenaBounds.width); Set(ai, "config.ArenaHeight", ArenaBounds.height);
