...
/// <summary>
/// Executes item-for-mana exchanges through one all-or-nothing transaction path shared by
/// player-to-NPC and NPC-to-NPC trading. It validates both inventories and Wallets before
/// mutation, commits item and mana ownership together, and records a correlated market entry.
///
/// Unity setup: none. This is a static service.
///   1. Use PlayerController2D or an inventory-enabled NpcController as participants.
///   2. Ensure each participant has a Wallet with enough balance/capacity.
///   3. Construct a TradeRequest and call TradeService.TryExecute(request).
///   4. SaveManager persists the bounded CompletedTrades ledger.
///
/// Runtime API:
...
///   Successful commits raise QuestEventBus "TradeCompleted" with itemId and quantity.
/// </summary>
public static class TradeService
{
    private const int MaxCompletedTrades = 500;
    private static readonly List<MarketTransaction> completedTrades = new();

    public static IReadOnlyList<MarketTransaction> CompletedTrades => completedTrades;
    ...
    /// both Wallets, transaction histories, and the market ledger unchanged.
    /// </summary>
    ...
    public static List<MarketTransaction> GetSaveData()
    {
        ...
        foreach (var transaction in completedTrades)
    ...
    public static void LoadSaveData(List<MarketTransaction> transactions)
    {
        ...
        int start = Mathf.Max(0, transactions.Count - MaxCompletedTrades);
    ...
    private static TradeResult Validate(TradeRequest request)
    {
        ...
        if (request.item == null || string.IsNullOrWhiteSpace(request.item.itemId))
            return TradeResult.Failed(TradeFailure.InvalidItem, "A stable ItemData itemId is required.");
        ...
        if (request.buyer.TradeWallet == null || request.seller.TradeWallet == null)
            return TradeResult.Failed(
                ...
                "Both participants require Wallets.");
            ...
            request.buyer.TradeWallet == request.seller.TradeWallet)
        {
            return TradeResult.Failed(
                ...
                "Buyer and seller cannot share an inventory or Wallet.");
        }
...
public sealed class TradeResult
{
    ...
    internal static TradeResult Valid() => new() { Success = true };

    internal static TradeResult Succeeded(MarketTransaction transaction) => new()
    {
        Success = true,
    ...
    internal static TradeResult Failed(TradeFailure failure, string message) => new()
    {
        Success = false,
        Failure = failure,
...
/// <summary>
/// Stable reason a TradeService request failed.
/// Unity setup: none.
/// </summary>
