...
public class NpcUseDoorBehavior : NpcBehaviorBase
{
    ...
    [SerializeField, Min(0.05f)] private float waypointThreshold = 0.2f;
    [Tooltip("How far past the gate the NPC walks after it opens.")]
    [SerializeField, Min(0.5f)] private float passThroughDistance = 2f;
    ...
    protected override void Enter()
    {
        ...
        gate = Perception != null
            ? Perception.FindNearestGate(Body.position, detectionRadius)
    ...
    private void TickApproach()
    {
        ...
        MoveToward(gate.transform.position, moveSpeed);
    }
    ...
    private void BeginPass()
    {
        ...
        passTarget = (Vector2)gate.transform.position + direction.normalized * passThroughDistance;
    ...
    private void TickPass()
    {
        ...
        if (Arrived(passTarget, waypointThreshold))
        {
            StopMoving();
            Complete();
            return;
        ...
        MoveToward(passTarget, moveSpeed);
    }
    ...
    private bool FollowPath(List<Vector2> waypoints, ref int index)
    {
        while (index < waypoints.Count)
        {
            ...
            if (Arrived(waypoint, waypointThreshold))
            {
                index++;
                continue;
            }

            MoveToward(waypoint, moveSpeed);
            return false;
        }
