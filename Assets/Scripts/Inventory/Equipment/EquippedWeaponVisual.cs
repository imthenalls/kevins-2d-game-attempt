...
public class EquippedWeaponVisual : MonoBehaviour
{
    ...
    [Tooltip("Attack-start angle around the owner. -20 places the grip at the lower-right.")]
    [SerializeField] private float startAngleOffset = -20f;
    [Tooltip("Attack-end angle around the owner. 200 places the grip at the lower-left.")]
    [SerializeField] private float endAngleOffset = 200f;
    [Tooltip("Makes the attack arc wider than the neutral holding radius.")]
    [SerializeField, Min(1f)] private float attackRadiusMultiplier = 1.25f;

    [Header("After-Swing Trail")]
    [SerializeField] private Color trailColor = new Color(1f, 0.05f, 0.03f, 0.85f);
    [SerializeField, Min(0.05f)] private float trailFadeTime = 0.28f;
    ...
    {
        return Mathf.Lerp(startAngleOffset, endAngleOffset, Smooth01(normalized));
    }
    ...
    private void ApplySwingPose(float attackAngle)
    {
        ...
        float attackRadius = restingOrbitPoint.magnitude * Mathf.Max(1f, attackRadiusMultiplier);
