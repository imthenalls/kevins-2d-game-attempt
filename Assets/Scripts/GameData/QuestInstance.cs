...
namespace Game.Core
{
    ...
    public class QuestInstance
    {
            ...
            foreach (var nodeId in snapshot)
            {
                ...
                foreach (var obj in node.objectives)
                {
                    ...
                    if (current < obj.requiredCount)
                        _objectiveCounts[obj.id] = Math.Min(current + amount, obj.requiredCount);
                }
            }
        ...
        private void EnterNode(string nodeId)
        {
            ...
            {
                QuestRuntimeBindings.Log($"[QuestInstance:{Graph.questId}] Node '{nodeId}' not found.");
                return;
            }
        ...
        private bool AllConditionsMet(QuestTransitionData transition)
        {
            ...
            {
                var condition = QuestRuntimeBindings.BuildCondition?.Invoke(condData);
