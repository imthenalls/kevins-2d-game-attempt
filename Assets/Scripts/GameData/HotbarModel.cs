...
{
    /// <summary>
    /// Pure-data model for the hotbar. Holds up to <see cref="SlotCount"/> item assignments (not
    /// copies — just which item type is "pinned" to each slot). Using a slot removes one of that
    /// item from the player's InventoryModel.
    ///
    ...
    /// Unity setup: none. Access at runtime via HotbarUI.Model.
    /// </summary>
    public class HotbarModel
    {
        public const int SlotCount = 6;

        private readonly IItem[] slots = new IItem[SlotCount];

        /// <summary>Fires after any assignment or clear.</summary>
        public event Action OnChanged;

        /// <summary>Returns the item assigned to <paramref name="index"/>, or null if empty.</summary>
        public IItem GetSlot(int index) =>
            (index >= 0 && index < SlotCount) ? slots[index] : null;

        /// <summary>Assigns <paramref name="item"/> to <paramref name="index"/>.</summary>
        public void Assign(int index, IItem item)
        {
